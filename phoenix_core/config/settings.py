...
# Load .env into the real process environment BEFORE any BaseSettings class is
# constructed. Without this, only the top-level Settings class (which has its
# own env_file=".env") sees the file's contents — every nested config class
# (TelegramConfig, GitHubConfig, ...) and the manual os.environ.get() calls in
# _populate_ai_providers_from_env only see truly exported env vars, so a valid
# .env file silently produces empty tokens/config everywhere except Settings itself.
load_dotenv()
...
class NewsConfig(BaseSettings):
    """Crypto news configuration (Task 020, /news). CryptoPanic requires a
    free API token (unlike Fear & Greed / fees, which need no key at all) —
    same "optional external integration" shape as GitHubConfig/TelegramConfig."""
    model_config = SettingsConfigDict(env_prefix="PHOENIX_NEWS_")

    enabled: bool = Field(default=True, description="Enable the /news command")
    api_token: SecretStr = Field(default=SecretStr(""), description="CryptoPanic API token")
    base_url: Optional[str] = Field(None, description="Override for the CryptoPanic API base URL")
    timeout: int = Field(default=15, ge=1, le=120, description="Request timeout in seconds")
    max_retries: int = Field(default=2, ge=0, le=10, description="Max retry attempts for transient failures")
    cache_ttl_seconds: float = Field(default=60.0, ge=1.0, description="How long cached responses stay valid")
